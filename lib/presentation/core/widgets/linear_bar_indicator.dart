...
class LinearBarIndicator extends StatelessWidget {
        ...
        StreamBuilder<DsDataPoint<double>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              ...
              log(LinearBarIndicator._debug, '[$LinearBarIndicator.build] error: ${snapshot.error}');
            } else if (snapshot.hasData) {
              final dataPoint = snapshot.data;
              // log(LinearBarIndicator._debug, '[$LinearBarIndicator.build] dataPoint: $dataPoint');
              if (dataPoint != null) {
  ...
  List<Widget> _buildValueWidgets(
    ...
    return [
        _buildValueTextBox(valueText, textBoxSize.width, textBoxSize.height, _cardPadding, valueTextStyle, value, lowColor, highColor),
        ...
        _buildLowIndicatorWidget(context, value, _strokeWidth, lowColor),
        _buildHighIndicatorWidget(context, value, _strokeWidth, highColor),
        _buildIndicatorWidget(
          value: value, 
          angle: _angle,
          color: color, 
  ...
  ///
  Widget _buildValueTextBox(String text, double width, double height, double padding, TextStyle textStyle, double value, Color lowColor, Color highColor) {
    final txt = text;
    if (txt.isNotEmpty) {
      return Positioned(
        width: width,
        ...
        child: Center(
          child: FittedBox(
            ...
            child: AnimatedDefaultTextStyle(
              ...
              style: _isLow(value)
                ? textStyle.copyWith(color: lowColor, fontWeight: FontWeight.w700,)
                : _isHigh(value)
                  ? textStyle.copyWith(color: highColor, fontWeight: FontWeight.w700,)
                  : textStyle,
  ...
  Widget _buildLowIndicatorWidget(
    BuildContext context, double value, double strokeWidth, Color lowColor,
  ) {
    final low = _low;
    if (low != null) {
    // log(_debug, '[$LinearBarIndicator._buildLowIndicatorWidget] strokeWidth: $strokeWidth');
      return _buildIndicatorWidget(
        value: _relativeValue.relative(low),
        strokeWidth: strokeWidth * 0.7,
        x: strokeWidth,
        angle: _angle,
        color: _isLow(value) ? lowColor : lowColor.withOpacity(0.4), 
  ...
  Widget _buildHighIndicatorWidget(
    BuildContext context, double value, double strokeWidth, Color highColor,
  ) {
    final high = _high;
    if (high != null) {
      ...
      return _buildIndicatorWidget(
        ...
        x: strokeWidth,
        angle: _angle + 180,
        color: _isHigh(value) ? highColor : highColor.withOpacity(0.4), 
        backgroundColor: Colors.transparent,
      );
    }
    return const SizedBox.shrink();
  ...
  double _boxWidth(double angle, double l, double minW) {
    ...
    // log(_debug, '[$LinearBarIndicator._boxWidth] boxWidth: $value');
  ...
  double _boxHeight(double angle, double l, double minH) {
    ...
    // log(_debug, '[$LinearBarIndicator._boxHeight] boxHeight: $value');
