...
class AppText {
  ...
  final String _value;
  ///
  const AppText(String value):
    _value = value;
  ///
  /// returns translations to the current [AppLang]
  String get local => to(appLng);
  String to(AppLang lng) {
    return tr(
      _value,
      lng: lng,
    );
  ...
  static String tr(String value, {AppLang? lng}) {
    final AppLang _lng = lng ?? appLng;
    if (_map.containsKey(value)) {
      final translations = _map[value];
      ...
      final _key = _map.keys.firstWhere(
        (key) {
    ...
    //   message: "[AppText] '$text' - перевод отсутствует",
    //   stackTrace: StackTrace.current,
    // );
    log(_debug, '[AppText.tr] нет перевода для "$value"');
    return value;
  }
}
